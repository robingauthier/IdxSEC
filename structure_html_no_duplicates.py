...
# taken from convert_html_text
# on va garder une liste en output
# mais l'idee c'est d'enlever les duplicatas


def convert_dict_to_text(resloc, meta=['elemi', 'tag', 'attrib']):
    """utility for converting the structure into a text for debugging"""
    ...
    if ('attrib' in meta) and ('attrib' in resloc.keys()):
...
def iter_html_no_duplicates(elem, elemi=0, lres=[], debug=0):
    """we mainly remove the style attribute that pollutes a lot
    the goal is to chunk the html document
    But I am not a big fan of this st
        ...
        if df_pandas is not None:
            ...
            if df_pandas1 is not None:
                ...
                if df_pandas1 is not None:
                    ...
                    resloc['score'] += 2
                    resloc['table']=df_pandas1

    if elem.tag not in ['table']:
        resloc['content'] += str(clean_txt(txtloc))
    ...
    if len(resloc['content'])>2000:
        resloc['score'] -= 1
    if len(resloc['content'])>4000:
        resloc['score'] -= 1
    if len(resloc['content'])>8000:
        resloc['score'] -= 1
    ...
    cres=[resloc]
    for child in elem:
        elemi,cresloc = iter_html_no_duplicates(child, elemi=elemi, lres=[], debug=debug)
        cres+=cresloc

    # ******logic for filtering duplicates******
    ncres=[resloc]
...
def structure_html_no_duplicates(fname):
    ...
    _,ltags = iter_html_no_duplicates(restree, elemi=0, lres=[], debug=3)
    return ltags
...
def test_run():
    ...
    txt = structure_html_no_duplicates(fname)
    with open(nfname, 'wt') as f:
        f.write(txt)

def test_1():
    fname='form14A_cik1850787_asof20230426_0001213900-23-032897.txt'
    structd = structure_html_no_duplicates(fname)
    lelems = [x['elemi'] for x in structd]
    assert 2013 in lelems,'issue element 2013 cannot be removed'
...
if __name__=='__main__':
    ...
    #     lstruct = structure_html_no_duplicates(fname)
